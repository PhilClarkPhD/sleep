...
def train_test_split(
    ...
    time_series_index: str = "epoch",
    group_col: str = "ID_day",
) -> tuple:
    """
    Splits data into train and test sets. Splitting is done equally across all values in the group_col.
    Splits done in time-series fashion - e.g. both train and test are continuous values wrt the time_series_index.
    ...
    Args:
        ...
        time_series_index (str): Column name to use to order rows prior to splitting train and test. Defaults to 'epoch'.
    ...
    df = df.sort_values(by=[group_col, time_series_index])
