"""
TODO:
- Make sure all widget definitions (e.g. Qlabel, QPushbutton, etc) are moved to initialize_objects.py
    - We only want to reference widgets through chain notation (self.Funcs.Display.widget)
-Create instance of Display() via self.funcs (e.g. label = self.funcs.Display.label)
-Use slot functions in update_objects to make instances functional
"""
...
class Tabs(QWidget):
    ...
    #
    #     epoch = self.Funcs.Display.epoch
    #
    ...
    #     # # #  TOP LAYOUT # # #
    #     # load data button
    #     button_layout.addWidget(self.Funcs.Display.load_data_button, 0, 0)
    #
    #     # load scores button
    ...
    #     # clear_scores_btn.clicked.connect(self.Funcs.clear_scores)
    #     button_layout.addWidget(clear_scores_btn, 0, 5)
    #
    #     # export scores as .txt
    #     export_scores_btn = QPushButton("Export Scores", self)
    #     # export_scores_btn.clicked.connect(self.Funcs.export_scores)
    ...
    #     # export_breakdown_btn.clicked.connect(self.Funcs.export_breakdown)
